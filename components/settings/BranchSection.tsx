...
import { Building2, CheckCircle, Edit2, MapPin, Phone, Plus, RefreshCw, Save, Trash2, X } from "lucide-react";
import { supabase } from "@/lib/supabase";

interface Branch { id: string; name: string; address: string; phone: string; is_active: boolean; }
...
export default function BranchSection({ disabled }: { disabled: boolean }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const { data } = await supabase.from("branches").select("id,name,address,phone,is_active").order("name");
  ...
  const openAdd = () => { setForm({name:"",address:"",phone:""}); setEditing(null); setAdding(true); };
  const openEdit = (b: Branch) => { setForm({name:b.name,address:b.address??""  ,phone:b.phone??""}); setEditing(b); setAdding(true); };

  const save = async () => {
    if (!form.name.trim()) { showToast(false,"Branch name is required."); return; }
    setSaving(true);
    if (editing) {
      const { error } = await supabase.from("branches").update({ name:form.name, address:form.address, phone:form.phone, updated_at:new Date().toISOString() }).eq("id", editing.id);
      if (error) showToast(false,"Failed to update branch."); else { showToast(true,"Branch updated."); setAdding(false); void load(); }
    } else {
      const { error } = await supabase.from("branches").insert({ name:form.name, address:form.address, phone:form.phone, is_active:true });
